...
def generate_launch_description():
    ...
    param_substitutions_two = {
        'use_sim_time': use_sim_time,
        'default_nav_to_pose_bt_xml': default_bt_xml_filename,
        'autostart': autostart
        }

    configured_params_two = ParameterFile( 
        RewrittenYaml(
            source_file=params_file,
            root_key=namespace,
            param_rewrites=param_substitutions_two,
            convert_types=True),
        allow_substs=True)
    ...
    declare_default_bt_xml_filename = DeclareLaunchArgument(
        'default_nav_to_pose_bt_xml', default_value=os.path.join(
                get_package_share_directory('turtlebot3_app_launcher'),
                'config', 'custom_bt.xml'),
    ...
    load_composable_nodes = LoadComposableNodes(
        ...
        composable_node_descriptions=[
            ...
            ComposableNode(
                ...
                name='bt_navigator',
                parameters=[configured_params_two],
                remappings=remappings),
            ComposableNode(
